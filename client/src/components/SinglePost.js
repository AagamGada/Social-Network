  ...
  const [showLikes, setShowLikes] = useState(false);
    ...
    const getPost = async () => {
      try {
        ...
        const data = await axios.get(`/Social-Hunt/api/post/${postId}`);
        setPost(data.data);
      } catch (err) {
        console.log(err);
      }
    ...
    const getComments = async () => {
      try {
        const postId = params.postId;
        const data = await axios.get(`/Social-Hunt/api/comment/${postId}`);
    ...
    async function getAllLikes() {
      try {
        const { data } = await axios.get(`/Social-Hunt/api/post/getlikes/${params.postId}`);
        setAllLikes([data]);
      } catch (err) {
        console.log(err);
      }
    ...
    const handleLike = async (ev) => {
      ev.preventDefault();
      try {
        const { data } = await axios.put(`/Social-Hunt/api/post/likes/${params.postId}`);
    ...
    const handleComment = async (ev) => {
      ...
      try {
        ...
        const postId = params.postId;
        const { data } = await axios.post(`/Social-Hunt/api/comment/${postId}`, {
          comment: comment,
        });
